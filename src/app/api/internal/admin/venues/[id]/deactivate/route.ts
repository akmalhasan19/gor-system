...
import { validateRequestBody, AdminVenueDeactivateSchema } from '@/lib/validation';
...
export async function POST(
    ...
    const validation = await validateRequestBody(request, AdminVenueDeactivateSchema);
    ...
    if (!beforeVenue.is_active) {
        return NextResponse.json({
            success: true,
            data: beforeVenue,
            message: 'Venue already inactive',
        });
    }
    ...
    if (error || !updatedVenue) {
        return NextResponse.json(
            { success: false, error: error?.message || 'Failed to deactivate venue' },
            { status: 500 }
        );
    }
